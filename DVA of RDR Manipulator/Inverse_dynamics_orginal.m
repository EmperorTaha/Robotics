...
M=[L2^2*M2+2*L1*L2*M2*cos(q2)+L1^2*(M1+M2) L2^2*M2+L1*L2*M2*cos(q2);L2^2*M2+L1*L2*M2*cos(q2) L2^2*M2];
V=[-M2*L1*L2*sin(q2)*dq2^2-2*M2*L1*L2*sin(q2)*dq1*dq2;M2*L1*L2*sin(q2)*dq1^2];
G=[M2*L2*g*cos(q1+q2)+(M1+M2)*L1*g*cos(q1);M2*L2*g*cos(q1+q2)];
T=M*[ddq1;ddq2]+V+G;
